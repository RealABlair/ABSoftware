...
namespace ABSoftware
{
    public class Memory32
    {
        ...
        public int FindSignature(byte[] signature, string mask, int minAddress = 0x11ffffff, int maxAddress = 0x7f000000)
        {
            ...
            while (address < maxAddress)
            {
                ...
                ReadProcessMemory(Handle, memory_basic_information.BaseAddress, lpBuffer, lpBuffer.Length, out int lpNumberOfBytesRead);
        ...
        public int[] FindSignatures(byte[] signature, string mask, int minAddress = 0x11ffffff, int maxAddress = 0x7f000000)
        {
            int[] sigs = new int[0];

            void AddSignature(int signatureAddress) { Array.Resize(ref sigs, sigs.Length + 1); sigs[sigs.Length - 1] = signatureAddress; }
                    ...
                    if ((lpBuffer[i] == signature[0] || mask[0] == '?') && (lpBuffer[i + 1] == signature[1] || mask[1] == '?'))
                    {
                        for (int j = 0; j < signature.Length; j++)
                        {
                            if ((lpBuffer[i + j] == signature[j]) || (mask[j] == '?'))
                            {
                                ...
                                if (num == signature.Length)
                                {
                                    AddSignature(((int)memory_basic_information.BaseAddress) + i);
                                    num = 0;
                                }
                            }
                            else
        ...
        public int[] FindValues<T>(T value, int minAddress = 0x11ffffff, int maxAddress = 0x7f000000)
        {
            Type type = typeof(T);
        ...
        public bool Read(int address, int length, out byte[] data)
        {
        ...
        public bool AllocateMemory(int address, uint size, MemoryProtection protection)
        {
            return VirtualAllocEx(Handle, (IntPtr)address, size, AllocationType.Commit | AllocationType.Reserve, protection) != IntPtr.Zero;
        }

        public bool AllocateMemory(int address, uint size, MemoryProtection protection, out IntPtr memoryBaseAddress)
        {
        ...
        public bool SetMemoryProtection(int address, uint size, MemoryProtection newProtection)
        {
            return VirtualProtectEx(Handle, (IntPtr)address, size, newProtection, out uint lpflOldProtect);
        }

        public bool FreeMemory(int address)
        {
